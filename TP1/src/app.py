...
# ✅ Mets ici le nom EXACT de ton checkpoint (présent sur le cluster)
...
# Si on change d'image, on reset les points (sinon c'est confus)
if st.session_state.get("last_img_name") != img_name:
    st.session_state["points"] = []
    st.session_state["last_img_name"] = img_name
...
# 7) Sélection du masque + affichage (hors du if do_segment)
...
if lp is not None and lp["img_name"] == img_name:
    ...
    st.write({
        "model_type": lp.get("model_type"),
        "bbox_xyxy": lp.get("box").tolist(),
    ...
    st.image(
        overlay,
        caption=f"mask_idx={idx} | score={float(scores[idx]):.3f} | time={lp.get('time_ms', 0.0):.1f} ms",
    ...
    st.write({
        ...
        "time_ms": float(lp.get("time_ms", 0.0)),
        "area_px": int(m_area),
        "mask_bbox": m_bbox,
    ...
    st.info("Cliquez sur 'Segmenter' pour générer des masques candidats.")
